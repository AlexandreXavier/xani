...
export class YamlCodeActions {
    ...
    getJumpToSchemaActions(diagnostics) {
        ...
        for (const schemaUri of schemaUriToDiagnostic.keys()) {
            const action = CodeAction.create(l10n.t('jumpToSchema', path.basename(schemaUri)), Command.create('JumpToSchema', YamlCommands.JUMP_TO_SCHEMA, schemaUri));
    ...
    getTabToSpaceConverting(diagnostics, document) {
        ...
        for (const diag of diagnostics) {
            if (diag.message === 'Tabs are not allowed as indentation') {
                ...
                result.push(CodeAction.create(l10n.t('convertToSpace'), createWorkspaceEdit(document.uri, [TextEdit.replace(resultRange, newText)]), CodeActionKind.QuickFix));
            }
        }
        if (result.length !== 0) {
            const replaceEdits = [];
            ...
            if (replaceEdits.length > 0) {
                result.push(CodeAction.create(l10n.t('convertAllSpaceToTab'), createWorkspaceEdit(document.uri, replaceEdits), CodeActionKind.QuickFix));
            }
        }
        return result;
    }
    getUnusedAnchorsDelete(diagnostics, document) {
        ...
        for (const diag of diagnostics) {
            if (diag.message.startsWith('Unused anchor') && diag.source === YAML_SOURCE) {
                ...
                const action = CodeAction.create(l10n.t('deleteUnusedAnchor', actual), createWorkspaceEdit(document.uri, [TextEdit.del(range)]), CodeActionKind.QuickFix);
        ...
        for (const diagnostic of diagnostics) {
            if (diagnostic.message === 'Incorrect type. Expected "boolean".') {
                ...
                if (value === '"true"' || value === '"false"' || value === "'true'" || value === "'false'") {
                    ...
                    results.push(CodeAction.create(l10n.t('convertToBoolean'), createWorkspaceEdit(document.uri, [TextEdit.replace(diagnostic.range, newValue)]), CodeActionKind.QuickFix));
                }
            }
        }
        return results;
        ...
        for (const diagnostic of diagnostics) {
            if (diagnostic.code === 'flowMap' || diagnostic.code === 'flowSeq') {
                ...
                if (isMap(node.internalNode) || isSeq(node.internalNode)) {
                    ...
                    results.push(CodeAction.create(l10n.t('convertToBlockStyle', 'Convert to block style {0}', blockTypeDescription), createWorkspaceEdit(document.uri, [TextEdit.replace(diagnostic.range, rewriter.write(node))]), CodeActionKind.QuickFix));
                }
            }
        }
        return results;
        ...
        for (const diagnostic of diagnostics) {
            if (diagnostic?.code === 'mapKeyOrder') {
                ...
                if (node && isMap(node.internalNode)) {
                    ...
                    results.push(CodeAction.create(l10n.t('fixKeyOrderToMap'), createWorkspaceEdit(document.uri, [TextEdit.replace(replaceRange, CST.stringify(sorted.srcToken))]), CodeActionKind.QuickFix));
                }
            }
        }
        return results;
    ...
    getQuickFixForPropertyOrValueMismatch(diagnostics, document) {
        ...
        for (const diagnostic of diagnostics) {
            ...
            for (const value of values) {
                results.push(CodeAction.create(String(value), createWorkspaceEdit(document.uri, [TextEdit.replace(diagnostic.range, String(value))]), CodeActionKind.QuickFix));
            }
        }
        return results;
    }
