...
`;
    ...
    it('should suggest enum based on type', async () => {
        ...
        const completion = await parseSetup(content, 0, content.length);
        (0, chai_1.expect)(completion.items.length).equal(2);
        (0, chai_1.expect)(completion.items[0].insertText).to.be.equal('"YES"');
        (0, chai_1.expect)(completion.items[1].insertText).to.be.equal('"NO"');
    });
    it('should suggest quotes with escapeChars', async () => {
        ...
        (0, chai_1.expect)(completion.items.length).equal(1);
        (0, chai_1.expect)(completion.items[0].insertText).to.be.equal('begin: "\\""');
        content = 'begin: ';
        completion = await parseSetup(content, 0, content.length);
        (0, chai_1.expect)(completion.items.length).equal(1);
        (0, chai_1.expect)(completion.items[0].insertText).to.be.equal('"\\""');
    });
});
