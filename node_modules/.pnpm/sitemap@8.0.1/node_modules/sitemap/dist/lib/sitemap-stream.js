...
function validateCustomNamespaces(custom) {
    ...
    if (custom.length > constants_1.LIMITS.MAX_CUSTOM_NAMESPACES) {
        throw new Error(`Too many custom namespaces: ${custom.length} exceeds limit of ${constants_1.LIMITS.MAX_CUSTOM_NAMESPACES}`);
    }
    ...
    for (const ns of custom) {
        ...
        if (ns.length > constants_1.LIMITS.MAX_NAMESPACE_LENGTH) {
            throw new Error(`Custom namespace exceeds maximum length of ${constants_1.LIMITS.MAX_NAMESPACE_LENGTH} characters: ${ns.substring(0, 50)}...`);
        }
        ...
        if (!xmlnsPattern.test(ns)) {
            throw new Error(`Invalid namespace format (must be xmlns:prefix="uri"): ${ns.substring(0, 50)}`);
        }
    }
}
...
class SitemapStream extends stream_1.Transform {
    constructor(opts = defaultStreamOpts) {
        ...
        if (opts.hostname !== undefined) {
            (0, validation_1.validateURL)(opts.hostname, 'hostname');
        }
        // Validate xslUrl if provided
        if (opts.xslUrl !== undefined) {
            (0, validation_1.validateXSLUrl)(opts.xslUrl);
        }
    ...
    _transform(item, encoding, callback) {
        ...
        if (!this.smiStream.write((0, validation_1.validateSMIOptions)((0, utils_1.normalizeURL)(item, this.hostname, this.lastmodDateOnly), this.level, this.errorHandler))) {
