...
function isImgAboveTheFold(img, imgElement, theFold) {
    const aboveTheFoldAttr = img.attr(ABOVE_FOLD_DATA_ATTR);
    if (aboveTheFoldAttr) {
...
async function processImage(htmlfile, img, isAboveTheFold) {
    var _a, _b, _c;
    /*
     * Attribute 'src'
     */
    ...
    if (!isLocal(attrib_src)) {
        switch (config.image.cdn.process) {
            ...
            case 'optimize':
                ...
                const new_srcset = await generateSrcSetForCdn(attrib_src, cdnTransformer, attrib_width, attrib_height);
                if (new_srcset !== null) {
        ...
        switch (config.image.external.process) {
            ...
            case 'download': // Download external image for local processing
                if (!isIncluded(attrib_src, config.image.external.src_include, config.image.external.src_exclude))
                    break;
                try {
    ...
    if (isAboveTheFold) {
        ...
        if ((_b = (await originalImage.getImageMeta())) === null || _b === void 0 ? void 0 : _b.isOpaque) {
...
async function _generateSrcSet(startSrc, imageWidth, imageHeight, transformSrc) {
...
async function generateSrcSetForCdn(startSrc, cdnTransformer, imageWidth, imageHeight) {
    return _generateSrcSet('', imageWidth, imageHeight, (valueW) => {
        var _a, _b;
