...
export const parse = (cacheControlHeader) => {
    const cacheControl = {};
...
export const stringify = (cacheControl) => {
    const directives = [];
    for (const [key, value] of Object.entries(cacheControl)) {
        if (!SUPPORTED_DIRECTIVES.includes(key))
            continue;
        switch (typeof value) {
            case 'boolean':
                directives.push(`${key}`);
